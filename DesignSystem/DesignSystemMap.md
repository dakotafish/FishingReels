...
These need a decision (mostly yours) before/while building — the design system doesn't resolve them:

1. **`--primary` semantic.** Should the shadcn/Button `--primary` map to **flame** (matches the CTA look but conflates with the live/accent role) or **ink** (safer, leaves flame for true CTAs/live)? The brand uses flame for *both* primary CTAs and live — need an explicit rule for non-CTA primaries.
2. **Destructive / danger token.** The brand has **no danger color** and says "compose only from the 8 swatches." What do destructive actions (delete, etc.) use — reuse flame (collides with CTA/live), introduce a new red (violates the palette rule), or rely on ink + confirmation copy? Needs a call.
3. **Dark mode.** Castline isn't a light/dark system. Do we (a) drop shadcn's `.dark` block entirely, (b) keep it for accessibility but design a Castline dark variant, or (c) treat "ink bands" as the only darkness? Currently `index.css` ships a full neutral dark theme that's unused-by-brand.
4. **Token format & namespace.** Convert Castline hex → oklch to match the existing file, or keep hex? And namespace brand tokens (`--cl-*`) to avoid the `--accent` collision with shadcn? Recommend oklch + `--cl-*` namespace, but confirm.
5. **Radius strategy.** Override shadcn's calc-based `--radius-*` to Castline's literal values (recommended), or pick a single `--radius` and accept drift? Affects every shadcn component's corners.
6. **Icon set.** Lucide is a *flagged substitution* — no brand icon set was provided. Stick with Lucide (and which delivery: CDN vs the `lucide-react` package, which is the better fit for this React build), or is a bespoke set coming? The fish emblem/wordmark are the only real marks shipped.
7. **Fonts in production.** Self-host the three TTFs (license OK to bundle?) and where (`public/fonts` vs `@fontsource`)? Variable TTFs are provided; confirm weight axes we actually need (Rethink 700/800, Epilogue 500/800 are the ones in use).
8. **Scope of screens vs the real data model.** Only the **homepage** was a finished design; Anglers list, Angler profile, and Live tournament are **original extensions pending brand review**. Also, the prototype's angler fixtures (`first/last/state/disciplines/tag/seasonLb/rank/bigBag/accent/disc/discText/photo/bio`) need reconciling with the actual backend **Angler** model/schema before wiring real data — confirm which fields exist server-side and how disciplines/stats map.
9. **Static `.cl-*` primitives vs shadcn components.** Do we port the primitives (`.cl-btn`, `.cl-card`, `.cl-badge-live`) as Tailwind/shadcn React components (recommended for consistency with `src/components/ui/`), or keep a parallel Castline CSS layer? Affects how `cn()` and variants are structured.
10. **The `--edge-gutter` vs `.wrap` duality.** Two competing horizontal systems (57px gutter for header/footer/livebar; 1240px+72px wrap for sections). Keep both (as the prototype does) or unify on one container strategy in the real app?
