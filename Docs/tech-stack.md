...
### MediaMTX — stream ingest + packager host
Custom image (`infra/mediamtx/Dockerfile`) pinned to `bluenviron/mediamtx:1.19.1-ffmpeg` (Alpine-based, ships ffmpeg) plus `curl`/`jq` for the hook scripts. Configured via `infra/mediamtx/mediamtx.yml`. Accepts SRT publishes on UDP 8890, where **the MediaMTX path is the angler's stream key** (`streamid=publish:<key>`).

- **Publish auth ("reject at the door").** `authMethod: http` — MediaMTX asks the backend (`/api/internal/mediamtx/auth`) before accepting any publish; unknown or revoked keys are refused at the SRT handshake and never touch disk. The callback URL carries the shared `MEDIAMTX_WEBHOOK_SECRET` and is injected via the `MTX_AUTHHTTPADDRESS` env var in compose, keeping the secret out of the committed config.
- **HLS packaging.** MediaMTX's own HLS muxer is **off**. `runOnReady` runs `scripts/on-ready.sh`: it registers the stream with the backend (creating the `Stream` row), then `exec`s an ffmpeg packager that remuxes the loopback RTSP feed (`-c copy`, no transcode, ~2–5% CPU) into **event-style fmp4 HLS** at `/recordings/<stream id>/` — 4s append-only segments, so viewers can scrub back to the very start of a live stream. When the publish stops, MediaMTX SIGINTs the packager; ffmpeg finalizes the playlist with `#EXT-X-ENDLIST` and the same files become the VOD. The accepted tradeoff: ~15–20s live latency for one player/one protocol. Publishers must send **H.264 + AAC** (pure remux — no HEVC).
- **Stream end.** `runOnNotReady` runs `scripts/on-not-ready.sh`: backstops the `#EXT-X-ENDLIST` if ffmpeg died ungracefully, then tells the backend to mark the stream `ended`.
- RTSP is loopback-only (`127.0.0.1:8554`, the packager's pull — never published); RTMP / WebRTC / MoQ are disabled. Stats API at port 9997 (dev only).

### Nginx — production edge
Custom multi-stage `Dockerfile` at `infra/nginx/Dockerfile`. Stage 1 (`node:22-alpine` builder) runs `npm ci --legacy-peer-deps` and `npm run build` against `apps/frontend/`; stage 2 (`nginx:alpine`) removes the stock `default.conf`, copies in `infra/nginx/nginx.conf` + `infra/nginx/conf.d/fishingreels.conf`, and copies the built `dist/` into `/usr/share/nginx/html`. Produces a single immutable image that serves the SPA at `/`, proxies `/api/*` to the FastAPI service via `upstream backend { server backend:8000; }` (compose DNS), and serves HLS at `/streams/*` from a read-only bind mount of `./data/hls/` at `/var/streams/`.

**HLS URLs are routed by file extension**: only `*.m3u8|m4s|mp4|ts` under `/streams/` is media — extension-less `/streams` and `/streams/<id>` are SPA page routes and fall through to `index.html`. Header policy: playlists are `Cache-Control: no-cache` (an event playlist grows while live); segments are `immutable` (an event playlist never rewrites one). The dev `hls` service (`infra/nginx/dev-hls.conf`) applies the same policy.
...
In dev, the edge nginx is **not** in the stack — Vite is the edge, proxying `/api/*` to the backend and `/streams/*.{m3u8,m4s,mp4,ts}` to a minimal `hls` nginx service (`nginx:1.27-alpine` + `infra/nginx/dev-hls.conf`, dev override only) that serves `./data/hls` read-only on port 8888. The prod stack is opt-in via `make prod-up` (which runs `docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build`); the dev `docker-compose.override.yml` is not auto-loaded when explicit `-f` flags are passed, so backend/postgres host ports are absent in prod and only nginx (`:80`) and MediaMTX SRT (`:8890/udp`) are publicly exposed.
